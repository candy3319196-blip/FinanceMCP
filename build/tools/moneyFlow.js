...
export const moneyFlow = {
    ...
    async run(args) {
        try {
            ...
            // 格式化输出
            const formattedOutput = formatMoneyFlowData(result.data, result.fields, queryType, args.ts_code);
            return {
...
// 格式化资金流向数据输出
function formatMoneyFlowData(data, fields, queryType, tsCode) {
    // 按交易日期倒序排列（最新在前）  
...
// 格式化板块资金流向表格
function formatSectorFlowTable(data) {
    let output = `## 📋 板块资金流向明细\n\n`;
